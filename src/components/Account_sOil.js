...
import{ 
  ...
  sOilDepositAmountSelector,
  sOilWithdrawAmountSelector
...
import {
	...
	sOilDepositAmountChanged,
	sOilWithdrawAmountChanged
} from '../store/actions'
 	...
 	return(
 		<Tabs defaultActiveKey= "deposit" className= "bg-dark text-white">
 			<Tab  eventKey= "deposit" title="Deposit" className="bg-dark">
				...
				<form className="row" onSubmit={(event) => {
				          event.preventDefault()
				          depositsOil(dispatch, exchange, web3, sOil, sOilDepositAmount, account)
				        }}>
				          <div className="col-12 col-sm pr-sm-2">
				            <input
				            type="text"
				            placeholder="sOil Amount"
				            onChange={(e) => dispatch( sOilDepositAmountChanged(e.target.value) )}
				            className="form-control form-control-sm bg-dark text-white"
				            required />
				          </div>
				          <div className="col-12 col-sm-auto pl-sm-0">
 			...
 			<Tab eventKey= "withdraw" title= "Withdraw" className= "bg-dark">
		        ...
		        <form className="row" onSubmit={(event) => {
		          event.preventDefault()
		          withdrawsOil(dispatch, exchange, web3, sOil, sOilWithdrawAmount, account)
		        }}>
		          <div className="col-12 col-sm pr-sm-2">
		            <input
		            type="text"
		            placeholder="sOil Amount"
		            onChange={(e) => dispatch( sOilWithdrawAmountChanged(e.target.value) )}
		            className="form-control form-control-sm bg-dark text-white"
		            required />
		          </div>
		          <div className="col-12 col-sm-auto pl-sm-0">
...
class Account extends Component {
	...
	async loadBlockchainData(){
		const {dispatch, web3, exchange, token, sUSD, sETH, sOil,  account } = this.props
		await loadBalances(dispatch, web3, exchange, token, sUSD, sETH, sOil, account)
	}
...
function mapStateToProps(state) {
	...
	return{
		...
		exchangesOilBalance: exchangesOilBalanceSelector(state),
		balancesLoading,
		...
		sOilDepositAmount: sOilDepositAmountSelector(state),
		sOilWithdrawAmount: sOilWithdrawAmountSelector(state),
